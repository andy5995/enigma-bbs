...
* **<kbd>U</kbd> to upload a reply packet reaches new installations only** ([#810](https://github.com/NuSkooler/enigma-bbs/issues/810)). Like every menu key, it is seeded from `misc/menu_templates/`, which `oputil config new` deploys once; your `menu.hjson` is yours and is never rewritten.
  ...
  **Action:** add the entry to the `submit` list of `messageBaseMainMenu` in your `menu.hjson`, and the menu it points at alongside your other message base menus:
  ...
  Your board accepts replies whether or not you add the key: `INF_HEADER.uses_upl_file` is now written as one, so every packet you export tells the reader to write its replies. Without the key there is no way to upload them.
...
* **Configuration problems are now reported at startup** ([#281](https://github.com/NuSkooler/enigma-bbs/issues/281)). A mistyped key in `config.hjson` has never been reported — because your file is merged *into* the defaults, a typo lands quietly beside the correct key and the setting you wrote is simply never read. Those are now listed on the console at startup and to the log on a hot reload.
  ...
  **Expect to see output you have not seen before**, possibly naming real mistakes that have been silently inert for years. This now covers `menu.hjson` and every theme as well as `config.hjson`, so a board with a long-customised menu file or a hand-edited theme should expect the most to say. **Nothing blocks**: a configuration with problems is applied exactly as before, and the board starts. Run `./oputil.js config validate` for the full report, and set `general.configValidation` to `off` to silence the startup output — the command keeps working either way. There is deliberately no strict mode.
...
* **A TIC could write its announced file outside the file base** ([#735](https://github.com/NuSkooler/enigma-bbs/issues/735)). The `File` field was checked for path traversal; the `Lfile` and `Fullname` fields, which are what a file is actually *stored* under, were not. A peer whose TIC otherwise validated — a configured node, a mapped `ticAreas` entry, a matching password if one is set, and a correct CRC — could send `Lfile ../../../some/path` and have ENiGMA½ write peer-supplied content to any path the BBS user could write. All three fields are now validated and such a TIC is rejected.

  **No action is required** beyond upgrading, and no configuration changes. If you carry TIC areas and want to be sure, check `paths.reject` for entries you do not recognise and look for unexpected files near your `fileBase.areaStoragePrefix`. Note that the `secureInOnly` change below independently narrows who could reach this at all.
