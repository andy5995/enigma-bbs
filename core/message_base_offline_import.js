...
//  produce a name for the message
const getMessageAreaField = areaTag => getMessageAreaByTag(areaTag) || {};
...
exports.getModule = class MessageBaseOfflineImport extends MenuModule {
    ...
    _readAndPersist(format, context, cb) {
        ...
        source.reader.on('message', message => pending.push(message));
    ...
    _resolveReplyTo(message, cb) {
        const replyToNumber = parseInt(
            _.get(message, 'meta.BlueWaveProperty.bw_reply_to_num'),
            10
        );
    ...
    _persistMessage(message, cb) {
        ...
        if (message.message.length > this.limits.maxMessageLength) {
            return cb(Errors.Invalid('Message is too long'));
        }
        ...
        //
        const area = getMessageAreaField(areaTag);
