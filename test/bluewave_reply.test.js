...
describe('Blue Wave reply packets', () => {
    ...
    describe('echotag routing', () => {
        ...
        //
        //  Two area tags that differ only past the twentieth character get
        //  the same base tag, and the writer steps the second one around the
        //  collision. The map has to step with it or those replies land in
        //  the first area.
        //
        it('follows the digit a collision forces onto the tail', () => {
            previousConfig = configModule._pushTestConfig({});
            ...
            assert.equal(map.get('A_VERY_LONG_AREA_TAG'), 'a_very_long_area_tag_one');
            assert.equal(map.get('A_VERY_LONG_AREA_TA2'), 'a_very_long_area_tag_two');
        });
    });
});
