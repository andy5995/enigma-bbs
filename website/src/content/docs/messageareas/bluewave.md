...
Replies written offline are uploaded back in, so a packet is two-way. `INF_HEADER.uses_upl_file` is written as one, which is how a door tells a reader to write its replies into a `*.UPL`.
